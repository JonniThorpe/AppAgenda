...
public class AgendaViewController implements Initializable{
   ...
   @Override 
    public void initialize(URL url, ResourceBundle rb){
        columnNombre.setCellValueFactory(new 
        ...
        columnProvincia.setCellValueFactory(new
            PropertyValueFactory<>("provincia"));
        columnProvincia.setCellValueFactory( cellData->{ 
            SimpleStringProperty property=new SimpleStringProperty();
            if (cellData.getValue().getProvincia()!=null){
                property.setValue(
                        cellData.getValue().getProvincia().getNombre()); 
            } return property;
    ...
    public void cargarTodasPersonas(){
        ...
        tableViewAgenda.setItems(
                FXCollections.observableArrayList(listPersona)); 
    }


    @FXML
    private void onActionButtonGuardar(javafx.event.ActionEvent event) {
        if (personaSeleccionada != null){
            ...
            TablePosition pos =
                    new TablePosition(tableViewAgenda,numFilaSeleccionada,null);
    ...
    private void onActionButtonNuevo(ActionEvent event){
        try{ // Cargar la vista de detalle 
            ...
            rootAgendaView.getScene().getRoot(); 
            rootMain.getChildren().add(rootDetalleView);
            personaDetalleViewController.setTableViewPrevio(tableViewAgenda);
            
        } catch (IOException ex){
            Logger.getLogger(AgendaViewController.class.getName()
            ).log(Level.SEVERE,null,ex); 
            } 
    }
    @FXML
    private void onActionButtonSuprimir(javafx.event.ActionEvent event) {
    }

    @FXML
    private void onActionButtonEditar(javafx.event.ActionEvent event) {
        try{ // Cargar la vista de detalle 
            FXMLLoader fxmlLoader=new 
            FXMLLoader(getClass().getResource("PersonaDetalleView.fxml"));
            Parent rootDetalleView=fxmlLoader.load();
